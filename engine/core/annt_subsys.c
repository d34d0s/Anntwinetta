#include "../include/annt_subsys.h"

void at_render_fallback(void* v){}
void at_events_fallback(void* v){}

bool at_render_main(void* dc) {
    if (!dc) return 1;
    at_draw_call* _dc = (at_draw_call*) dc;
    at_render_subsys* _rsys = (at_render_subsys*) _dc->rsys;
...
bool at_init_render_subsys(int mode, at_vec3 ccolor, at_render_subsys* rsys) {
    ...
    rsys->_proc = at_new_proc((void*)0, at_render_main, at_default_fallback);
    return 0;
}

bool at_events_main(void* v) {
    at_event_t* _event  = (at_event_t*) v;
    while (SDL_PollEvent(&_event->event)) {
        switch (_event->event.type) {
            case SDL_QUIT:
                _event->type = at_quit_event;
                break;
            case SDL_KEYDOWN:
                _event->type = at_key_down_event;
                _event->data.key.keyCode = _event->event.key.keysym.scancode;
                break;
            case SDL_KEYUP:
                _event->type = at_key_up_event;
                _event->data.key.keyCode = _event->event.key.keysym.scancode;
                break;
            case SDL_MOUSEMOTION:
                _event->type = at_mouse_motion_event;
            ...
            case SDL_MOUSEBUTTONDOWN:
                if (_event->event.button.button <= MAX_MOUSE_BUTTONS) {
                    at_mouse_state[_event->event.button.button - 1].isPressed = true;
                }
                _event->type = at_mouse_button_down_event;
            ...
            case SDL_MOUSEBUTTONUP:
                if (_event->event.button.button <= MAX_MOUSE_BUTTONS) {
                    at_mouse_state[_event->event.button.button - 1].isPressed = false;
                }
                _event->type = at_mouse_button_up_event;
                _event->data.mouseButton.mouseX = _event->event.button.x;
                _event->data.mouseButton.mouseY = _event->event.button.y;
                _event->data.mouseButton.button = _event->event.button.button;
                break;
            case SDL_MOUSEWHEEL:
                at_mouse_state->scrollValue += _event->event.wheel.y;
                _event->type = at_mouse_wheel_event;
                _event->data.mouseWheel.scrollDirection = _event->event.wheel.y;
                break;
            case SDL_CONTROLLERBUTTONDOWN:
                _event->type = at_controller_button_down_event;
                _event->data.controller.controllerButton = _event->event.cbutton.button;
                break;
            case SDL_CONTROLLERBUTTONUP:
                _event->type = at_controller_button_up_event;
                _event->data.controller.controllerButton = _event->event.cbutton.button;
                break;
            default:
                _event->type = at_none_event;
